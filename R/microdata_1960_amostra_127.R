...
apply_corrections_1960_amostra_127 <- function(linhas, correcoes,
  ...
  if(nrow(externos)){
    ...
    for(i in seq_along(reparos)){
      ...
      if(any(substr(textos127, 3, 6) != substr(card25, 30, 33)) ||
         any(substr(textos127, 7, 8) != substr(card25, 34, 35)) ||
         any(substr(textos127, 18, 18) != substr(card25, 36, 36)))
...
recover_family_cards_1960_amostra_127 <- function(tabelas,
  ...
  for(i in seq_along(cartoes)){
    ...
    if(any(substr(textos25, 1, 8) != paste0(regra$pasta, regra$boletim)))
      stop("boletim da fonte25 divergente")
    seguinte <- raw[as.character(regra$linha_25 + nrow(dec) + 1L)]
    if(!is.na(seguinte) && substr(seguinte, 1, 8) == paste0(regra$pasta, regra$boletim))
    ...
    for(var in names(p127)){
      if(!identical(p127[[var]], pessoas[[var]][idx]) ||
         (var %in% comuns && !identical(p127[[var]], p25[[var]])))
...
# ignorada (V204=9) entra nas faixas que a incluem explicitamente (quadros 1
# e 2); nos quadros 3, 4 e 5, a soma conhecida exclui idade ignorada e a
# convencao pendente torna a comparacao parcial, sem presumir sua elegibilidade.
#
...
diagnosticar_universos_1960_amostra_127 <- function(pessoas, domicilios){
  ...
  motivos_dom <- list(tipo_domicilio_desconhecido = !d$V101 %in% c(1:5, 9),
...
validate_1965_1960_amostra_127 <- function(tabelas, gabarito_path,
  ...
  n1 <- rbind(q1[, somar_validacao_1960_amostra_127(w), by = .(regiao, linha)][, coluna := "total"],
              ...
              q1[!is.na(situacao), somar_validacao_1960_amostra_127(w), by = .(regiao, linha, coluna = paste0(situacao, "_", sexo))])
  ...
  n2 <- rbind(q2[, somar_validacao_1960_amostra_127(w), by = .(regiao, linha)][, coluna := "total"], q2[, somar_validacao_1960_amostra_127(w), by = .(regiao, linha, coluna = sexo)],
              ...
              q2[alf != "sem_declaracao", somar_validacao_1960_amostra_127(w), by = .(regiao, linha, coluna = paste0(alf, "_", sexo))])[, quadro := 2L]
  ...
  q3[V220 %in% 2:3 & !is.na(V223B), ramo := data.table::fifelse(V223 %in% 4:5, "outras_atividades", data.table::fifelse(V223B < 200, "agricultura_pecuaria_silvicultura",
  ...
  n4 <- rbind(q3[, somar_validacao_1960_amostra_127(w), by = .(regiao, linha = renda)][, coluna := "total"], q3[, somar_validacao_1960_amostra_127(w), by = .(regiao, linha = renda, coluna = paste0(grupo, "_", sexo))],
              q3[, somar_validacao_1960_amostra_127(w), by = regiao][, `:=`(linha = "TOTAIS", coluna = "total")], q3[, somar_validacao_1960_amostra_127(w), by = .(regiao, coluna = paste0(grupo, "_", sexo))][, linha := "TOTAIS"])[, quadro := 4L]
  ...
  n5 <- rbind(q5[, somar_validacao_1960_amostra_127(w), by = .(regiao, linha)][, coluna := "total"],
    q5[!is.na(grupo_dependencia), somar_validacao_1960_amostra_127(w),
      by = .(regiao, linha, coluna = paste0(grupo_dependencia, "_", sexo))])[, quadro := 5L]
  ...
  for(faixa_pendente in c(FALSE, TRUE)) for(situacao_pendente in c(FALSE, TRUE)){
    marcar(grade$quadro == 1L & (grade$linha != "TOTAIS") == faixa_pendente &
           ...
           presenca_incerta | (p$presente & ((faixa_pendente & p$idade_danificada) |
  ...
  for(alf_pendente in c(FALSE, TRUE)){
    marcar(grade$quadro == 2L & grepl("sabem", grade$coluna) == alf_pendente,
           presenca_incerta | (p$presente & (p$idade_danificada |
             (alf_pendente & (p$idade >= 5 | p$idade_ignorada) & !p$V211 %in% 0:4))))
  }
  base_10 <- presenca_incerta | (p$presente & (p$idade_danificada | p$idade_ignorada))
  ramo_incerto <- p$presente & p$idade >= 10 & (!p$V220 %in% 0:9 |
    (p$V220 %in% 2:3 & is.na(p$V223B) & !p$V223 %in% 4:5))
  grupo_incerto <- p$presente & p$idade >= 10 & (!p$V220 %in% 0:9 |
  ...
  for(renda_pendente in c(FALSE, TRUE)) for(grupo_pendente in c(FALSE, TRUE)){
    marcar(grade$quadro == 4L & (grade$linha != "TOTAIS") == renda_pendente &
           ...
           base_10 | (renda_pendente & p$presente & p$idade >= 10 & !p$V219 %in% 0:9) |
  ...
  for(nm in names(quesitos)) for(pessoas_pendentes in c(FALSE, TRUE)) for(situacao_pendente in c(FALSE, TRUE)){
    incerto <- !d$V101 %in% c(1:5, 9) | (d$V101 %in% c(1, 2, 4, 5) & !d$somente_nao_moradores &
      (d$ocupacao_incerta | (pessoas_pendentes & d$n_presenca_incerta > 0L) |
  ...
  comp[, politica_idade := data.table::fifelse(quadro == 1L, "ignorada declarada na faixa final publicada",
    data.table::fifelse(quadro == 2L, "ignorada declarada na faixa final e agregados 5+/10+/15+ por convencao de divulgacao",
      data.table::fifelse(quadro %in% 3:5, "idade ignorada/danificada: inclusao pendente nesta fonte; soma conhecida parcial", "sem limite etario")))]
...
validate_definitivos_1960_amostra_127 <- function(tabelas, definitivos_path,
  ...
  comp <- data.table::rbindlist(lapply(c("censobr_weight", "censobr_weight_1965"), function(peso){
    ...
    presenca_incerta <- !p$V202 %in% 1:6
    marcar(grade$tabela %in% c(32L, 33L, 34L, 37L), presenca_incerta)
    marcar(grade$tabela == 33L & !grade$item %in% c("total", "ignorada"),
      (presenca_incerta & !p$idade_ignorada) | (p$presente & p$idade_danificada))
    marcar(grade$tabela == 33L & grade$item == "ignorada", (presenca_incerta & p$idade_ignorada) |
      ((p$presente | presenca_incerta) & !p$V204 %in% c(0, 1, 5, 9)))
    marcar(grade$tabela == 34L & grade$item != "total", presenca_incerta | (p$presente & !p$V118 %in% c(1, 3, 5)))
    marcar(grade$tabela == 37L & grade$item != "total", presenca_incerta | (p$presente & !p$V206 %in% 4:9))
    ...
    for(pessoas_pendentes in c(FALSE, TRUE)) for(situacao_pendente in c(FALSE, TRUE)){
      incerto <- (!d$V101 %in% c(1:5, 9) | d$V101 %in% c(1, 2, 4, 5)) & !d$V102 %in% 6 & !d$somente_nao_moradores &
        (!d$V101 %in% c(1:5, 9) | !d$V102 %in% 4:5 | d$ocupacao_incerta |
