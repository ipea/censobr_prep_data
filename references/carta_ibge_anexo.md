...
**Pedido.** Saber se essa publicação chegou a existir, e se há no arquivo do Instituto — ainda que como documento interno — a descrição dos estratos e das frações de sorteio da subamostra de 1,27%; e se os cartões perfurados ou as fitas originais dessa amostra foram preservados. Sem isso, os estratos têm de ser reconstruídos por aproximação a partir do próprio arquivo, o que produz erros-padrão conservadores, e o quarto grupo do desenho (cidades de 100.000 habitantes e mais) não é reconstruível.
