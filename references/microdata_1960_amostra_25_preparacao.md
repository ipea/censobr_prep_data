...
- **O desacordo com a amostra de 1,27%** nas variáveis do domicílio — 8,65% no tipo de construção — fica medido e não resolvido. A compilação o contorna sem decidi-lo: como ela é uma partição por unidade da federação, em cada unidade vale uma transcrição só, e nenhum registro publicado depende de escolher entre as duas. A matriz continua como referência.
...
- **A compilação das duas amostras** está feita, no bloco `# 01c.` — ver [`microdata_1960_compilacao.md`](microdata_1960_compilacao.md). Estas dezessete unidades entram nela inteiras; as outras onze vêm da amostra de 1,27%.
