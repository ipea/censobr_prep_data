...
- **Município.** `censobr_muni_1960` é V116 levado à divisão territorial de 1960 (`read_guides/1960_municipios.csv`, gerado da DTB de 1960 e do Anuário Estatístico do Brasil de 1961, com nome, código de 2010, zona fisiográfica e população total, urbana e rural de cada um dos 2.769 municípios). É igual a V116 em 24 UFs. Na Guanabara, V116 é o distrito (5411 a 5581) e o município é um só, 541, o Rio de Janeiro; em Alagoas, V116 vem deslocado em 200 (Maceió é 2506 no arquivo e 2306 na divisão territorial, e os 14 municípios da amostra caem na faixa certa, com Maceió 100% urbana e os rurais do Anuário em pastas rurais); em Fernando de Noronha vem 2701, e o território tem um município só, 2401. `censobr_muni_corrigido` marca as três UFs. Ficam sem correspondência a Serra dos Aimorés (5001: a região em litígio que a divisão territorial reparte em vários municípios), um código de São Paulo com um domicílio (7234) e um domicílio do Paraná sem município — lixo de linhas danificadas.
...
|---|---|---|---|
| `pessoas_1960_amostra_127.parquet` | 897.009 | 72 | 36 MB |
| `domicilios_1960_amostra_127.parquet` | 174.245 | 40 | 5 MB |
...
| `censobr_muni_1960`, `censobr_muni_corrigido` | o código de município na divisão territorial de 1960 (`read_guides/1960_municipios.csv`) e a marca das três UFs em que ele difere de V116 (passo 8) |
...
- **Três classes de atividade fora do Código**: 424 (116 pessoas), 429 (1.077) e 716 (679) ocorrem no arquivo e não estão no exemplar do Código do Censo — 716 está numa linha cortada pela digitalização, 424 e 429 não constam da página do comércio, que termina em 423. O padrão do Código sugere 429 = atividades não compreendidas nas classes anteriores. Um exemplar oficial resolveria (pergunta na carta ao IBGE).
