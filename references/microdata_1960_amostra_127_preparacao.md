...
- **Município.** `code_muni_1960` é V116 levado ao Código de Municípios e Distritos de 1960, e `code_muni` o código atual pelo crosswalk da mesma tabela (`read_guides/1960_municipios.csv`, gerado da DTB de 1960 e do Anuário Estatístico do Brasil de 1961, com nome, código de 2010, zona fisiográfica e população total, urbana e rural). V116 é o código oficial em 24 UFs. Nas outras três, a correção é determinística e tem fonte: em Alagoas o arquivo traz 25xx onde o Código dá 23xx (Maceió é 2506 no arquivo e 2306 no Código; os 14 municípios da amostra caem na faixa certa, Maceió 100% urbana e os rurais do Anuário em pastas rurais); em Fernando de Noronha traz 2701 onde o Código dá 2401, único município do território; na Guanabara V116 é o bairro (5410 a 5581) e a cidade é um município só, 541 por convenção da DTB, sem código de município no censo porque o estado era a cidade. `censobr_muni_corrigido` marca as três UFs. Dois domicílios tinham código sem par no Código: um de São Paulo com 7234 (código que não existe; as duas linhas do boletim o trazem, e as outras 222 famílias da mesma pasta são 6234, São Paulo) e um do Paraná com "724Z" (um caractere corrompido em três linhas seguidas; as outras 276 famílias da pasta são 7240). Como a pasta é sorteada dentro do município e não há outra leitura possível, `code_muni_1960` recebe o município da pasta, marcado; V116 fica como está. A Serra dos Aimorés não tem município atual: é a região em litígio entre Minas e Espírito Santo, que o Código de Municípios de 1960 traz como uma unidade só, 5001 ("divisão adotada para efeito do Recenseamento Geral de 1960"; também assim na amostra de 25%), e que a divisão territorial reparte em vários municípios hoje capixabas. `code_muni_1960` guarda o 5001 oficial e `code_muni` recebe 99, o código do polígono "Litígio MG/ES" da malha de 1960 do `geobr`, para que a coluna não fique vazia e cruze com o mapa. Os três cartões de pessoa cujo V116 era ilegível ("724Z", anulado no passo 4) recebem em V116 o município deduzido pela pasta, marcados. Com isso nenhuma pessoa fica sem V116, `code_muni_1960` ou `code_muni`.
- **Distrito, bairro e favela.** `code_district_1960` é o campo de distrito da chave do questionário (posições 7 e 8), que é o código de distrito do Código de Municípios e Distritos de 1960; `name_district_1960` é o nome que o livro dá a esse código, por `read_guides/1960_distritos.csv`. A tabela foi transcrita do livro em três partes: os distritos de todos os municípios por OCR (Tesseract com o modelo português e as coordenadas de cada palavra; o código de município conferido pelo nome na DTB, porque o datilografado confunde o 4 com h e L; o distrito 01 recebe o nome do município, que é a sede), os 84 bairros, as circunscrições e as 147 favelas da Guanabara por leitura visual das doze páginas (`name_bairro_1960` e `censobr_favela` só existem para o Rio), e as unidades especiais da p. 312 (Brasília com os distritos 01 a 06, Fernando de Noronha, Serra dos Aimorés). Os 119 pares município-distrito que o OCR não resolveu foram lidos à mão nas páginas em 2026-09-15, bloco a bloco: 227 municípios relidos, 765 linhas conferidas na imagem da página, marcadas na coluna `fonte`. Cobertura: 174.235 dos 174.245 domicílios têm o nome do distrito. Os dez que faltam são um domicílio cada, com código que não existe no livro (Gravatá com uma letra no lugar do dígito, Inhapim com um código par, quatro da Guanabara com bairro inválido): é dano de perfuração, não falta de transcrição, e ficam listados em `read_guides/1960_distritos_pendentes.csv`. Na amostra, 900 domicílios cariocas estão em favelas identificadas nominalmente (a Série Especial de 1965 tem um volume só sobre elas).
...
|---|---|---|---|
| `pessoas_1960_amostra_127.parquet` | 897.009 | 77 | 36 MB |
| `domicilios_1960_amostra_127.parquet` | 174.245 | 45 | 5 MB |
...
Duas colunas dizem como o sorteio foi feito: `censobr_upa` é a pasta, e `censobr_estrato` é a unidade da federação cruzada com o grupo de situação da pasta — os quatro grupos do desenho de 1965: cidade de 100 mil habitantes ou mais, aglomerado urbano menor, mista, rural. São 817 pastas em 75 estratos (64 de uma unidade da federação só e 11 que juntam duas ou três vizinhas, onde o estrato ficaria com uma pasta só), de 2 a 54 pastas cada, mediana 7; o único com uma pasta é Fernando de Noronha, e é de propósito — ali a pasta sorteada era o cadastro inteiro.
...
Com isso, o passo 11 calcula o erro-padrão de cada estimativa somando, dentro de cada estrato, a dispersão dos totais entre as pastas, com a correção de população finita de uma pasta em vinte (e fração 1 em Fernando de Noronha, onde não houve sorteio de pasta). A essa parcela soma-se a parte da etapa anterior — um domicílio em quatro — que a dispersão entre pastas não alcança: o estimador entre pastas contém 95% dela, e o 1/20 que falta acrescenta de 0,04% a 0,6% ao erro-padrão. A primeira parcela é o que `survey::svydesign(ids = ~censobr_upa, strata = ~censobr_estrato, weights = ~censobr_weight, fpc = ~fpc)` daria, escrita à mão para não acrescentar dependência ao pipeline; o `survey` está no renv e confere o resultado em `references/conferencia_desenho_amostral_1960.R`. O resultado, com o peso final, para a população presente:

| domínio | estimativa | erro-padrão | CV | pelos resíduos da calibração | CV |
|---|---|---|---|---|---|
| Brasil | 70.094.960 | 550.338 | 0,79% | 5.618 | 0,01% |
| Brasil, urbana | 31.572.374 | 482.716 | 1,53% | 53.292 | 0,17% |
| Brasil, rural | 38.522.586 | 581.462 | 1,51% | 52.995 | 0,14% |
| Leste | 24.570.761 | 305.989 | 1,25% | 0 | 0,00% |
| Sul | 24.605.438 | 337.029 | 1,37% | 0 | 0,00% |
| Nordeste | 15.486.240 | 263.581 | 1,70% | 0 | 0,00% |
| Norte e Centro-Oeste | 5.432.521 | 161.800 | 2,98% | 5.618 | 0,10% |

São duas contas, não duas versões da mesma. A primeira ignora a calibração e responde "quanto esta amostra erraria se nada se soubesse sobre os pesos". A segunda é a variância dos resíduos da regressão nas 543 células da calibração, e só vale para `censobr_weight`, cujos totais são externos (a contagem completa em onze unidades da federação, a amostra de 25% nas outras dezessete): ela dá zero nas margens que a calibração prendeu — as regiões, fora o Distrito Federal — e o que sobra é o que não foi restrição, isto é, o Distrito Federal (5.618 no total do país) e o corte urbano/rural das unidades da federação com menos de oito pastas. Para `censobr_weight_1965` as duas colunas de resíduo ficam vazias, porque os totais de 1965 saíram desta mesma amostra e a conta devolveria zero sem significado.

Nas 176 células do quadro 1 (região × situação × sexo × faixa de idade), o coeficiente de variação tem mediana de 3,5% e máximo de 11,8%, este nas células pequenas do Norte e Centro-Oeste; pelos resíduos da calibração, mediana de 1,3% e máximo de 5,7%. O efeito de desenho — quantas vezes a amostra é menos precisa que um sorteio pessoa a pessoa do mesmo tamanho — tem mediana 6,2 nessas células, e chega a 249 quando o domínio é definido por região e situação, porque a pasta é quase toda urbana ou quase toda rural e as pastas são justamente o que se sorteia. Dito de outro modo: para estimar quanta gente morava na zona urbana do Nordeste, esta amostra vale 817 sorteios, não 885 mil.

Tudo em `data_raw/microdata/1960/amostra_127/erros_amostrais.csv`, uma linha por domínio e por peso (coluna `peso`), com estimativa, erro-padrão de desenho, coeficiente de variação, efeito de desenho, número de pessoas e as duas colunas de resíduo da calibração.
