...
- **[produto] O produto difere do publicado em v0.5.0.** `name_region` muda de "Centro-oeste" para "Centro-Oeste" em 276.692 linhas (todas as do Centro-Oeste); seis colunas `code_*` mudam de tipo (`int32` em v0.5.0). As outras 51 colunas são idênticas: 4.024.543 linhas, 58 colunas na mesma ordem, 0 NA em `code_muni`, 4.491 municípios, soma de pesos 35.435.725. (Afirmava-se "nenhuma divergência".)
...
- **[produto] A correção elimina 85 colunas espúrias**, não 86: sem ela, a união dos nomes de Pessoa02 dá 256 colunas (`V001`–`V255` + `V1005`); com ela, 171. Nenhum parquet publicado ou construído teve 256 colunas `pessoa02_V*`: todos têm 2.008 colunas, 310.114 linhas e exatamente 171.
...
- **[produto]** `code_weighting` existe só nos produtos de 2000 e 2010. 1970, 1980 e 1991 não a têm, e 1960 não tem `code_muni`, `code_state` nem `code_region` — só `code_muni_1960`. A ausência em 2022 rompe com 2000/2010, não com "todas as edições".
