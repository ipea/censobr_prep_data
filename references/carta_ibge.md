...
**Anexo:** `carta_ibge_anexo.md` — 69 itens, com arquivo afetado, evidência medida e data de verificação
...
São 69 itens: 16 de gravidade alta, 22 média e 31 baixa. Os de gravidade
...
2. **1980.** A republicação em DBF (janeiro de 2025) não contém o Território
...
3. **1991.** A republicação em DBF não traz o identificador do questionário
...
6. **Agregados por setor de 2022.** A variável `V0006` do Básico (domicílios
...
7. **Transversal.** Só três arquivos de log de atualização existem em toda a
...
Nosso pedido é duplo: a correção, ou a republicação a partir de cópias
íntegras, dos arquivos dos itens de gravidade alta; e a manutenção de um log
de atualização por edição, como o IBGE já faz para os microdados de 2000 e
